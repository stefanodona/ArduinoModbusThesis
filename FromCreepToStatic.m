...
t_rises = zeros(length(x_long),1);
measTime = 0.1;

acc = json.acc_max*50*1e-3;
dec = json.acc_max*5*1e-3;
vel = json.vel_max*5*1e-3;

for ii = 2:length(x_long)
    if (ii<=3)
        t_1 = getTime(abs(x_long(ii)), acc,dec,vel);
        t_rises(ii) = t_1;
        if (ii==2)
            t_x_rise(ii) = t_1 + t_x_fall(ii-1);
            t_x_fall(ii) = t_x_rise(ii) + measTime + json.wait_time*1e-3;
        else
            t_x_rise(ii) = 2*t_1 + t_x_fall(ii-1);
            t_x_fall(ii) = t_x_rise(ii) + measTime + json.wait_time*1e-3;
        end
    else
        t_1 = getTime(abs(x_long(ii)), acc,dec,vel);
        t_rises(ii) = t_1;
        if(mod(ii,2)==0)
            t_x_rise(ii) = 2*t_1 + t_x_fall(ii-1) + json.wait_time*1e-3;
            t_x_fall(ii) = t_x_rise(ii) + measTime + json.wait_time*1e-3;
        else
            t_x_rise(ii) = 2*t_1 + t_x_fall(ii-1);
            t_x_fall(ii) = t_x_rise(ii) + measTime + json.wait_time*1e-3;
...
%     force_neg = -force_neg;
    ...
    force(1:num_of_zeros_rise) = 0;
    force_neg = circshift(force_neg, int64(num_of_zeros_fall));
    force_neg(1:num_of_zeros_fall) = 0;
